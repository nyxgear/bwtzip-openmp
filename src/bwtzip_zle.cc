...
namespace {
    void encodeLengthWheeler(vector<unsigned char>& v, size_t length) {
        length++;                           // Increment length first
    
        vector<unsigned char> temp;
    
        while (length != 1) {               // Do not write most significant bit
...
vector<unsigned char>& bwtzip::unzleWheeler(vector<unsigned char>& v) {
    ...
    for (size_t i = 0; i < v.size(); i++) {
        ...
        if (byte <= 0x01) {
            if (run == false) { // We must ghostwrite the most significant bit to be 1
                length = 1;
                run = true;
            }
            ...
            run = false;
            length--;   // Decrement length to undo the effect of the Wheeler ZLE
