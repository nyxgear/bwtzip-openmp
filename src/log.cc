...
void Log::pbwtzip::started(int iter, int bs) {
    if (LOG_BWTZIP && LOG_STAGE) printf("[PBWTZIP][pbwtzip] ---- Started iteration %d, BS %d\n", iter, bs);
}
...
void Log::pbwtzip::print_total_exec_time(double time) {
    if (LOG_PBWTZIP) printf("[PBWTZIP][pbwtzip] Execution time: %f\n\n", time);
    if (LOG_STATISTICS_CSV) csv::csv_line << time;
}
 ...
 ****************** PBWTZIP2 *******************
 */
void Log::pbwtzip2::started(int iter, int bs) {
    if (LOG_BWTZIP && LOG_STAGE) printf("[PBWTZIP][pbwtzip2] ---- Started iteration %d, BS %d\n", iter, bs);
}

void Log::pbwtzip2::ended(int iter, int bs) {
    if (LOG_PBWTZIP && LOG_STAGE) printf("[PBWTZIP][pbwtzip2] ---- Ended iteration %d, BS %d\n\n", iter, bs);
}


void Log::pbwtzip2::stats_print_summary(int iter, int lasted_longer_count[], double time_averages[]) {
    if (LOG_STATISTICS) {
        string stage[5] = {"read_file  ", "stage_1    ", "stage_2    ", "write_file "};
...
void Log::pbwtzip2::print_total_exec_time(double time) {
    if (LOG_PBWTZIP) printf("[PBWTZIP][pbwtzip2] Execution time: %f\n\n", time);
    if (LOG_STATISTICS_CSV) csv::csv_line << time;
}


void Log::pbwtzip2::stage::started(string stage_name, int bs) {
    if (LOG_PBWTZIP && LOG_STAGE) printf("[PBWTZIP][%s] Started. BS: %d\n", stage_name.data(), bs);
}


void Log::pbwtzip2::stage::ended(string stage_name, double time) {
    if (LOG_PBWTZIP && LOG_STAGE) printf("[PBWTZIP][%s] Ended. Time of stage execution: %f\n", stage_name.data(), time);
}

void Log::pbwtzip2::stage::chunk_read(string stage_name, string buffer_name, int bs, int i, void *c) {
    if (LOG_PBWTZIP && LOG_STAGE && LOG_CHUNKS) {
        std::string nll = "";
        if (c == nullptr)
            nll = "NULL ";
...
void Log::pbwtzip2::stage::chunk_written(string stage_name, string buffer_name, int bs, int i, void *c) {
    if (LOG_PBWTZIP && LOG_STAGE && LOG_CHUNKS) {
        std::string nll = "";
        if (c == nullptr)
            nll = "NULL ";
